...
def giris_zorunlu(f):
    ...
    def wrap(*args, **kwargs):
        if "kullanici_id" not in session:[cite: 1]
...
def sinav_oturumunu_temizle():
    ...
    for key in anahtarlar:[cite: 1]
        session.pop(key, None)[cite: 1]
...
def auzef_harfsiz_ve_harfli_soru_ayikla(metin, unite_no=1):
    temiz = re.sub(r'about:blank\s*\d+/\d+', '', metin)[cite: 1]
    temiz = re.sub(r'\d{1,2}\.\d{1,2}\.\d{4}\s+\d{1,2}:\d{1,2}', '', temiz)[cite: 1]
    temiz = re.sub(r'Ders:\s*.*?(?:\n|\|)', '', temiz, flags=re.IGNORECASE)[cite: 1]
    temiz = re.sub(r'Ünite:\s*.*?\n', '', temiz, flags=re.IGNORECASE)[cite: 1]

    bloklar = re.split(r'(?:^|\n)\s*Soru\s*[-–—:]*\s*(\d{1,2})\s*:', temiz, flags=re.IGNORECASE)[cite: 1]
    sorular = [][cite: 1]
    if len(bloklar) > 1:[cite: 1]
        for i in range(1, len(bloklar), 2):[cite: 1]
            s_no = bloklar[i].strip()[cite: 1]
            icerik = bloklar[i+1].strip()[cite: 1]

            cevap_ara = re.search(r'(?:Cevap\s*[-–—:]*\s*\d{0,2}\s*:|Doğru\s*Cevap\s*:)\s*([^\n\r]+)', icerik, flags=re.IGNORECASE)[cite: 1]
            cevap_metni = cevap_ara.group(1).strip() if cevap_ara else ""[cite: 1]

            govde = icerik[:cevap_ara.start()].strip() if cevap_ara else icerik[cite: 1]
            govde = re.sub(r'\(Çoktan Seçmeli\)', '', govde, flags=re.IGNORECASE).strip()[cite: 1]

            satirlar = [s.strip() for s in govde.split("\n") if s.strip()][cite: 1]
            if not satirlar:[cite: 1]
                continue[cite: 1]

            if len(satirlar) >= 6 and not re.match(r'^\(?[A-Ea-e]\)?', satirlar[-1]):[cite: 1]
                sec_e, sec_d, sec_c, sec_b, sec_a = satirlar[-1], satirlar[-2], satirlar[-3], satirlar[-4], satirlar[-5][cite: 1]
                soru_kok = " ".join(satirlar[:-5])[cite: 1]
            else:
                sec_a_m = re.search(r'(?:\(A\)|A\)|A\.-)\s*(.*?)(?=(?:\([B-E]\)|[B-E]\)|[B-E]\.-))', govde, re.DOTALL)[cite: 1]
                sec_b_m = re.search(r'(?:\(B\)|B\)|B\.-)\s*(.*?)(?=(?:\([C-E]\)|[C-E]\)|[C-E]\.-))', govde, re.DOTALL)[cite: 1]
                sec_c_m = re.search(r'(?:\(C\)|C\)|C\.-)\s*(.*?)(?=(?:\([D-E]\)|[D-E]\)|[D-E]\.-))', govde, re.DOTALL)[cite: 1]
                sec_d_m = re.search(r'(?:\(D\)|D\)|D\.-)\s*(.*?)(?=(?:\(E\)|E\)|E\.-|\Z))', govde, re.DOTALL)[cite: 1]
                sec_e_m = re.search(r'(?:\(E\)|E\)|E\.-)\s*(.*?)$', govde, re.DOTALL)[cite: 1]

                if sec_a_m and sec_b_m and sec_c_m:[cite: 1]
                    soru_kok = govde[:sec_a_m.start()].strip()[cite: 1]
                    sec_a = sec_a_m.group(1).strip()[cite: 1]
                    sec_b = sec_b_m.group(1).strip()[cite: 1]
                    sec_c = sec_c_m.group(1).strip()[cite: 1]
                    sec_d = sec_d_m.group(1).strip() if sec_d_m else "-"[cite: 1]
                    sec_e = sec_e_m.group(1).strip() if sec_e_m else "-"[cite: 1]
                else:
                    soru_kok = satirlar[0][cite: 1]
                    sec_a = satirlar[1] if len(satirlar) > 1 else "-"[cite: 1]
                    sec_b = satirlar[2] if len(satirlar) > 2 else "-"[cite: 1]
                    sec_c = satirlar[3] if len(satirlar) > 3 else "-"[cite: 1]
                    sec_d = satirlar[4] if len(satirlar) > 4 else "-"[cite: 1]
                    sec_e = satirlar[5] if len(satirlar) > 5 else "-"[cite: 1]

            val_a, val_b, val_c, val_d, val_e = " ".join(sec_a.split()), " ".join(sec_b.split()), " ".join(sec_c.split()), " ".join(sec_d.split()), " ".join(sec_e.split())[cite: 1]
            dogru_harf = "A"[cite: 1]
            c_clean = " ".join(cevap_metni.split()).lower()[cite: 1]
            tek_harf = re.search(r'^[A-Ea-e]$', c_clean.strip())[cite: 1]
            if tek_harf:[cite: 1]
                dogru_harf = tek_harf.group(0).upper()[cite: 1]
            else:
                for harf, val in [("A", val_a), ("B", val_b), ("C", val_c), ("D", val_d), ("E", val_e)]:[cite: 1]
                    if val != "-" and (val.lower() == c_clean or val.lower() in c_clean or c_clean in val.lower()):[cite: 1]
                        dogru_harf = harf[cite: 1]
                        break[cite: 1]
    ...
    return sorular[cite: 1]

def klavuz_pdf_ayikla(pdf_bytes):
    """
    Kılavuz PDF metnini sayfa sayfa okur, '1. ÜNİTE / 1. BAŞLIK' gibi numaralandırmaları
    algılayarak maddeleri ünite ünite gruplar.
    """
    ...
    for satir in satirlar:
        if "FAITH S. AKADEMİ" in satir or "TELEGRAM" in satir or "KILAVUZU" in satir:[cite: 2]
            continue
...
def otomatik_klavuz_isle():
    ...
    try:
        ...
        session["bildirim"] = {"tur": "success", "metin": f"Harika! '{ders}' için {len(ayiklanan_uniteler)} ünite ve toplam {toplam_eklenen} hap bilgi başarıyla sisteme aktarıldı."}
...
def yukle_unite_sorulari():
    ders = request.form.get("ders_adi", "").strip()[cite: 1]
    unite_no = int(request.form.get("unite_no", 1))[cite: 1]
    dosya = request.files.get("soru_dosyasi")[cite: 1]

    if not dosya or not dosya.filename.lower().endswith(".pdf"):[cite: 1]
        session["bildirim"] = {"tur": "danger", "metin": "Lütfen soru PDF'i seçin."}[cite: 1]
        return redirect(url_for("icerik_merkezi", ders=ders))[cite: 1]

    metin = ""[cite: 1]
    try:
        reader = PdfReader(io.BytesIO(dosya.read()))[cite: 1]
        for page in reader.pages:[cite: 1]
            y = page.extract_text()[cite: 1]
            if y:[cite: 1]
                metin += y + "\n"[cite: 1]
    except Exception as e:
        session["bildirim"] = {"tur": "danger", "metin": f"PDF okunamadı: {str(e)}"}[cite: 1]
        return redirect(url_for("icerik_merkezi", ders=ders))[cite: 1]

    sorular = auzef_harfsiz_ve_harfli_soru_ayikla(metin, unite_no)[cite: 1]
    if not sorular:[cite: 1]
        session["bildirim"] = {"tur": "warning", "metin": f"PDF okundu ancak {unite_no}. üniteye ait sorular algılanamadı."}[cite: 1]
        return redirect(url_for("icerik_merkezi", ders=ders))[cite: 1]

    conn = veritabani_baglan()[cite: 1]
    cursor = conn.cursor()[cite: 1]
    cursor.execute("DELETE FROM sorular WHERE TRIM(ders_adi) = TRIM(?) AND unite_no = ?", (ders, unite_no))[cite: 1]

    for s in sorular:[cite: 1]
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, '', ?)
        """, (ders, s["metin"], s["a"], s["b"], s["c"], s["d"], s["e"], s["dogru_cevap"], s["aciklama"], unite_no))[cite: 1]

    conn.commit()[cite: 1]
    conn.close()[cite: 1]

    session["bildirim"] = {"tur": "success", "metin": f"Tebrikler! {ders} - Ünite {unite_no} için {len(sorular)} soru aktarıldı."}[cite: 1]
...
def hizli_soru_ekle():
    ders = request.form.get("ders_adi", "").strip()[cite: 1]
    unite_no = int(request.form.get("unite_no", 1))[cite: 1]
    ham_metin = request.form.get("soru_metinleri", "")[cite: 1]

    if not ham_metin.strip():[cite: 1]
        session["bildirim"] = {"tur": "warning", "metin": "Lütfen soru metinlerini yapıştırın."}[cite: 1]
        return redirect(url_for("icerik_merkezi", ders=ders))[cite: 1]

    sorular = auzef_harfsiz_ve_harfli_soru_ayikla(ham_metin, unite_no)[cite: 1]
    if not sorular:[cite: 1]
        session["bildirim"] = {"tur": "danger", "metin": "Metin çözümlenemedi. Lütfen formatı kontrol edin."}[cite: 1]
        return redirect(url_for("icerik_merkezi", ders=ders))[cite: 1]
    ...
    session["bildirim"] = {"tur": "success", "metin": f"'{ders}' - Ünite {unite_no} için {len(sorular)} soru başarıyla kaydedildi."}[cite: 1]
    return redirect(url_for("unite_pekistirme_listesi", ders=ders))[cite: 1]
...
def unite_test_baslat(unite_no):
    ...
    cursor.execute("""
        ...
        WHERE TRIM(ders_adi) LIKE ? AND unite_no = ?
        ORDER BY id ASC
    """, (f"%{ders}%", unite_no))[cite: 1]
    satirlar = cursor.fetchall()[cite: 1]
    conn.close()[cite: 1]

    if not satirlar:[cite: 1]
        session["bildirim"] = {"tur": "warning", "metin": f"'{ders}' dersinin {unite_no}. ünitesine ait soru bulunamadı. Lütfen 'İçerik Yükle' alanından soru PDF'ini yükleyin."}[cite: 1]
        return redirect(url_for("unite_pekistirme_listesi", ders=ders))[cite: 1]

    sorular = [dict(r) for r in satirlar][cite: 1]
    sinav_oturumunu_temizle()[cite: 1]

    session["sorular"] = sorular[cite: 1]
    session["aktif_ders"] = f"{ders} (Ünite {unite_no} Pekiştirme)"[cite: 1]
    session["sinav_modu"] = "ogrenme"[cite: 1]
    session["mevcut_indeks"] = 0[cite: 1]
    session["cevaplar"] = [][cite: 1]
    session["dogru"] = 0[cite: 1]
    session["yanlis"] = 0[cite: 1]
    session["bos"] = 0[cite: 1]
    session["toplam_sure_saniye"] = 0[cite: 1]

    return redirect(url_for("soru_goruntule"))[cite: 1]
...
def sinav_baslat():
    ...
    if ozel_havuz == "yildizli":[cite: 1]
        cursor.execute("SELECT * FROM sorular WHERE yildizli = 1")[cite: 1]
    ...
    if not satirlar and ders != "HEPSI":[cite: 1]
        conn = veritabani_baglan()[cite: 1]
        cursor = conn.cursor()[cite: 1]
        cursor.execute("SELECT * FROM sorular WHERE TRIM(ders_adi) LIKE ?", (f"%{ders}%",))[cite: 1]
        satirlar = cursor.fetchall()[cite: 1]
        conn.close()[cite: 1]
        aktif_ders_adi = ders[cite: 1]
    ...
    tum_sorular = [dict(row) for row in satirlar][cite: 1]
    ...
    session["sorular"] = secilen_sorular[cite: 1]
    session["aktif_ders"] = aktif_ders_adi[cite: 1]
    session["sinav_modu"] = mod[cite: 1]
    session["mevcut_indeks"] = 0[cite: 1]
    session["cevaplar"] = [][cite: 1]
    session["dogru"] = 0[cite: 1]
    session["yanlis"] = 0[cite: 1]
    session["bos"] = 0[cite: 1]
    session["toplam_sure_saniye"] = sure_dakika * 60[cite: 1]
...
@giris_zorunlu
def soru_goruntule():
    sorular = session.get("sorular", [])[cite: 1]
    ...
    if indeks >= len(sorular):[cite: 1]
        return redirect(url_for("sonuc_goruntule"))[cite: 1]

    soru = sorular[indeks][cite: 1]

    conn = veritabani_baglan()[cite: 1]
    cursor = conn.cursor()[cite: 1]
    cursor.execute("SELECT yildizli, kullanici_notu, unite_no FROM sorular WHERE id = ?", (soru["id"],))[cite: 1]
    row = cursor.fetchone()[cite: 1]
    if row:[cite: 1]
        soru["yildizli"] = row["yildizli"][cite: 1]
        soru["kullanici_notu"] = row["kullanici_notu"] or ""[cite: 1]
        soru["unite_no"] = row["unite_no"] or 0[cite: 1]
    conn.close()[cite: 1]
    ...
    if request.method == "POST":[cite: 1]
        ...
        if mod == "sinav":[cite: 1]
            if session["mevcut_indeks"] >= len(sorular):[cite: 1]
                return redirect(url_for("sonuc_goruntule"))[cite: 1]
...
def testi_bitir():
    ...
    for i in range(indeks, len(sorular)):[cite: 1]
        s = sorular[i][cite: 1]
        cevap_listesi.append({
            "soru": s,
            "secilen": "Boş",
    ...
    session["cevaplar"] = cevap_listesi[cite: 1]
    session["mevcut_indeks"] = len(sorular)[cite: 1]
    return redirect(url_for("sonuc_goruntule"))[cite: 1]
