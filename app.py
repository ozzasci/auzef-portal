...
def sinav_oturumunu_temizle():
    for key in ["soru_idleri", "sorular", "aktif_ders", "sinav_modu", "mevcut_indeks", "cevaplar", "dogru", "yanlis", "bos"]:
...
def giris_yap():
    if request.method == "POST":
        ...
        session["bildirim"] = {"tur": "danger", "metin": "Hatalı kullanıcı adı veya şifre."}
...
# Çıkmış soruların koyu şıklarını okuyan ayrıştırıcı
def auzef_cikmis_soru_ayikla(metin, unite_no=1):
    ...
    for i in range(1, len(bloklar), 2):
        ...
        # Doğru şık tespiti (Metinde kalın veya işaretli gelme durumları)
        dogru_harf = "A"
...
def sinav_baslat():
    ...
    if not satirlar:
        session["bildirim"] = {"tur": "warning", "metin": "Seçilen kritere uygun soru bulunamadı. Lütfen İçerik Merkezi'nden soru PDF'i yükleyin."}
        return redirect(url_for("ana_sayfa"))
    ...
    session["aktif_ders"] = f"{ders} (Sınav Simülasyonu)"
